...
coap_crypto_read_pem_private_key(const char *filename,
  ...
  pk_ctx_init = 1;
#if defined(MBEDTLS_2_X_COMPAT) || defined(COAP_USE_PSA_CRYPTO)
  C(mbedtls_pk_parse_keyfile(pk_ctx, filename, NULL));
#else
  C(mbedtls_pk_parse_keyfile(pk_ctx, filename, NULL, coap_rng));
#endif /* !(MBEDTLS_2_X_COMPAT || COAP_USE_PSA_CRYPTO) */
...
#else
  C(mbedtls_pk_parse_key(pk_ctx, binary->s, binary->length, NULL, 0, coap_rng, NULL));
#endif /* !(MBEDTLS_2_X_COMPAT || COAP_USE_PSA_CRYPTO) */
...
static int
fix_public_key_info(mbedtls_pk_context *pk_ctx, coap_crypto_cred_t *local) {
  int ret;
  uint8_t buf[200];
...
coap_crypto_read_pem_public_cred(const char *filename,
  ...
  mbedtls_pk_context *pk_ctx = mbedtls_malloc(sizeof(mbedtls_pk_context));
  coap_crypto_cred_t *local = mbedtls_malloc(sizeof(coap_crypto_cred_t));
  int pk_ctx_init = 0;
...
void
coap_crypto_delete_public_cred(coap_crypto_cred_t *public) {
  if (public) {
    ...
    coap_delete_bin_const(public->auth_cred);
    mbedtls_free(public);
  }
}
...
coap_crypto_hash_sign(cose_alg_t hash,
  ...
  C(mbedtls_pk_sign(pk_ctx,
                    ...
                    NULL,
                    NULL));
#elif defined(COAP_USE_PSA_CRYPTO)
  C(mbedtls_pk_sign(pk_ctx,
                    hash_alg,
                    text->s,
                    text->length,
...
coap_crypto_hash_verify(cose_alg_t hash,
  ...
  size_t hash_length;
  mbedtls_md_type_t hash_alg = get_hash_alg(hash, &hash_length);
  mbedtls_pk_context *pk_ctx = (mbedtls_pk_context *)public_cred->key_tls;
